    ...
    <>
      ...
      <nav className="bottom-nav">
        {TABS.map(tab => {
          ...
          return (
            <button key={tab.k} className={'nav-btn'+(active?' active':'')} onClick={() => setPage(tab.k)} aria-label={tab.l[lang]||tab.l.PT}>
              ...
              <span className="nav-icon" style={{display:'flex',alignItems:'center',justifyContent:'center'}}>{NAV_SVG[tab.nav]}</span>
              <span className="nav-label">{tab.l[lang]||tab.l.PT}</span>
            </button>
          )
        })}
        ...
        <button className={'nav-btn'+(isMorePage||moreOpen?' active':'')} onClick={() => setMoreOpen(o => !o)} aria-label="Mais serviços">
          ...
          <span className="nav-icon" style={{display:'flex',alignItems:'center',justifyContent:'center'}}>{NAV_SVG.more}</span>
          <span className="nav-label">{lang==='EN'?'More':lang==='ES'?'Más':lang==='FR'?'Plus':lang==='DE'?'Mehr':'Mais'}</span>
        </button>
