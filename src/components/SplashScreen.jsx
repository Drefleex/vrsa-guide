...
export default function SplashScreen({ lang, setLang, onStart }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'linear-gradient(175deg,#002D55 0%,#003B6F 55%,#004F96 100%)', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'40px 32px', overflow:'hidden', fontFamily:"'Inter',-apple-system,sans-serif" }}>
      ...
      <div style={{ width:88, height:88, borderRadius:16, background:'rgba(255,255,255,.1)', border:'1.5px solid rgba(255,255,255,.2)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:44, marginBottom:24, animation:ready?'inst-logo .6s cubic-bezier(.34,1.56,.64,1) both':'none', opacity:ready?undefined:0 }}><img src="/brasao-vrsa.png" alt="Brasão VRSA" style={{ width:72, height:72, objectFit:'contain' }} /></div>
