          ...
          {SPOTS.map((s,i,arr) => (
            <div key={s.id} style={{ padding:'14px 16px', borderBottom:i<arr.length-1?'1px solid var(--surface)':'none' }}>
              ...
              <div style={{ display:'flex', gap:8, alignItems:'center', marginLeft:56 }}>
                ...
                <button aria-label={t.navigate} onClick={() => window.location.assign('https://www.google.com/maps/search/?api=1&query='+s.lat+','+s.lng)} style={{ marginLeft:'auto', padding:'3px 10px', background:'var(--surface)', border:'1px solid var(--border)', borderRadius:50, fontSize:10, fontWeight:700, color:'var(--ink-40)', cursor:'pointer' }}>📍 {t.navigate}</button>
              </div>
            </div>
          ))}
        </div>
