          ...
          {HEALTH.map((h,i,arr) => (
            <div key={i} style={{ padding:'13px 16px', borderBottom:i<arr.length-1?'1px solid var(--surface)':'none' }}>
              ...
              <div style={{ display:'flex', gap:8, marginLeft:56 }}>
                ...
                {h.lat !== 0 && (
                  <button onClick={() => window.location.assign(`https://www.google.com/maps/dir/?api=1&destination=${h.lat},${h.lng}`)} style={{ padding:'7px 12px', background:'var(--surface)', border:'1px solid var(--border)', borderRadius:9, fontSize:12, fontWeight:700, color:'var(--ink-40)', cursor:'pointer' }}>🧭</button>
                )}
              </div>
            </div>
          ))}
